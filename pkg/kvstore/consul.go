...
func (c *consulModule) setDummyConfig() {
...
func (c *consulModule) newClient() (KVClient, error) {
	if c.config == nil {
...
func (c *consulClient) LockPath(path string) (kvLocker, error) {
	...
	for retries := 0; retries < maxLockRetries; retries++ {
		...
		case err != nil:
			return nil, err
		case ch == nil && err == nil:
			Trace("Acquiring lock timed out, retrying", nil, log.Fields{fieldKey: path, logfields.Attempt: retries})
		default:
			return lockKey, err
		}
	}
...
func (c *consulClient) Watch(w *Watcher) {
	...
	for {
		...
		if err != nil {
			...
			Trace("List of Watch failed", err, log.Fields{fieldPrefix: w.prefix, fieldWatcher: w.name})
		}
