...
func (a *Allocator) lockedAllocate(key AllocatorKey) (ID, bool, error) {
	...
	if value != 0 {
		...
		if err != nil {
			return 0, false, fmt.Errorf("no more identifiers available")
		}
	...
	if id != oldID {
		...
		return 0, false, fmt.Errorf("local key conflict")
	}
	...
	if err != nil {
		...
		a.localKeys.release(k)
		lock.Unlock()
		return 0, false, fmt.Errorf("maser key CAS operation failed: %s", err)
	}
