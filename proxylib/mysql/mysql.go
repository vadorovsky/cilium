...
func (rule *mysqlRule) Matches(data interface{}) bool {
	...
	p := sqlparser.New()
	tree, warns, err := p.Parse(sql, "", "")
	for _, warn := range warns {
		log.WithError(warn).Warning("peoblem when parsing query")
	}
	if err != nil {
		log.WithError(err).Warning("could not parse query")
		return false
	}
	...
	if len(rule.queryActionExact) > 0 && rule.queryActionExact != "" {
		log.Debugf("mysqlRule: cmd mismatch %s, %s", rule.queryActionExact, "")
		return false
	}
	if rule.tableRegexCompiled != nil &&
		!rule.tableRegexCompiled.MatchString("") {
		log.Debugf("mysqlRule: file mismatch %s", rule.tableRegexCompiled.String())
		return false
	}
